...
先确定项目是纯 Theme 工作，还是包含商品、内容和店铺资源的完整建站。完整建站项目在首次安装项目应用时申请覆盖常见建站工作的权限包，避免每进入下一阶段就增加 scope、重新部署和授权。记录“工作范围 → 所需能力 → 执行应用身份”；后续出现建站权限包以外的新需求再补充。
...
`Shopify-Template` 本身没有 `shopify.app.toml`。如果项目只做 Theme，无须为了候选权限创建应用；如果预计要用 Admin GraphQL 管理店铺数据，先创建或关联**本项目自己的 Shopify App**，确认其 Client ID 和目标店铺，再将确定的 scopes 写入该应用的 `shopify.app.toml`。运行 `shopify app config validate --json`，部署配置并完成目标店铺的授权。记录计划、配置声明和安装后实际获批的三份 scope 清单。应用定义的 Metaobject / Metafield schema 优先写入 `shopify.app.toml`，由所属应用部署；scope 不能代替资源所有权。
...
上面的 `write_products` 只是计划写入商品时的示例；实际校验项必须来自本项目的操作清单。模板校验脚本按返回的 scope 名称做精确匹配；当 Shopify 允许 write scope 读取对应资源时，不要仅凭缺少 `read_*` 名称判定读取能力不足。不要只根据 `shopify.app.toml` 或 CLI deploy 的成功状态判断 scope 已生效，必须使用**同一个项目应用身份**读取 `currentAppInstallation.accessScopes { handle }`。如果缺 scope：

1. 先确认目标 store 和 App identity 没有错。
2. 修改配置、运行 `shopify app config validate --json` 并执行 `shopify app deploy`；非交互环境如需 `--allow-updates`，先确认将部署的版本和目标应用。
3. 由有权限的店铺人员批准新增 scope，通过正常 OAuth 重新授权后再次校验。AI 不得把“已写入 TOML”或“部署成功”报告为“权限已获得”。
4. 不要换用另一个项目的 token 绕过权限问题。

数据资源工作使用项目所属应用的 Admin GraphQL 身份；Shopify CLI 用于应用配置、部署和 Theme 生命周期。`shopify app execute` 的 mutation 仅适用于开发店，不要作为正式店的通用写入方案。使用 `shopify store auth` / `shopify store execute` 前，先确认其应用身份适合目标资源，尤其不能用另一身份管理项目应用拥有的 Metaobject 定义。
