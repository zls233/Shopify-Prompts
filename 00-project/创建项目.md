...
如果预计本项目会导入商品、建立分类或创建 Pages / Blogs / Articles，先执行 `00-project/建站授权准备.md`，完成一次完整建站权限包的申请、安装和真实权限核对。后续资源任务复核其结果，不重复创建应用。本节是初始化摘要；授权操作和完成标准以该独立 Prompt 为准。
...
在任何 Shopify Admin API 写入之前，必须明确当前 store 和 App identity。店铺自有资源运行：
